...
Adding the argument `sort.by.sig=T`, sorts the x-axis so that the most significant transcription factors are on the left.

```{r, fig.width=10, fig.height=8, out.width = 750, out.height = 600, message=F}
plot(monsterRes, rescale=T)
```
...
```{r, fig.width=10, fig.height=8, out.width = 750, out.height = 600, message=F}
library(gplots)
heatmap.2(monsterRes@tm, col = "bluered",
density.info="none",  
trace="none", 
dendrogram='none',     
Rowv=FALSE,
...
```{r, fig.width=10, fig.height=8, out.width = 750, out.height = 600, message=F}
  require(ggplot2)
  tm.pca <- princomp(monsterRes@tm)
  odsm <- apply(monsterRes@tm,2,function(x){t(x)%*%x})
